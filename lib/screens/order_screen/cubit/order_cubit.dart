...
class OrderCubit extends Cubit<OrderState> {
  ...
  OrderCubit() : super(const OrderHomeInitial(message: "ORDER LIST IS EMPTY!", iconData: Icons.list)) {
    _loadOrders();
  }
  ...
  void searchOrder(String search) {
    List<OrderModel> searchOrders = orders.where((order) => order.productname!.toLowerCase().startsWith(search.toLowerCase())).toList();

    if (searchOrders.isNotEmpty) {
      emit(OrderSearchState(orderModel: searchOrders));
    } else {
      emit(const OrderFoundState(message: "ORDER NOT FOUND!", iconData: Icons.search_off_rounded));
    }
  }
}
